...
z_vec = np.linspace(start=20.,stop=0.,num=4,endpoint=False)[::-1]
...
plt.plot(m_calc_an,z_vec,label=r'$\mathbf{m}_{calculated}$',color='orange',lw=3,ls='dashed')
plt.plot(m_calc_noise,z_vec,label=r'$\mathbf{m}_{noisy}$',color='cyan',lw=1.)

plt.ylim(z_vec[-1],0.)
plt.xlim(1.e-03,1.e-01)
plt.xlabel('Slowness [s/m]')
plt.ylabel('depth [m]')
plt.xscale('log')

plt.title('Slowness vs. Depth: \nAnalytical vs. Simple Collocation (coarse sensor array)')

plt.grid()
plt.legend()
plt.show()
#plt.savefig('haynes_hw2_p3e.png',dpi=250)
